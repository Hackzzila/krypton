...
class Chain extends Transform {
   ...
   * @returns {Promise<T>|T} - The unwraped return value of the last function in the chain
   */
  run(async, data) {
    if (async === undefined) {
    ...
    if (this.chunker) {
      if (this.chunker.name === 'Chunker::Opus') {
        ...
        while (this.buffer.length >= required * (n + 1)) {
          this.push(await this.run(true, this.buffer.slice(n * required, (n + 1) * required)));
          n += 1;
        }
      ...
      return callback(null, await this.run(true, chunk));
    }
